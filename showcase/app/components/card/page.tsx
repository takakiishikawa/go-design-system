    ...
    <div className="flex flex-col gap-8 py-4">
      ...
      <DemoSection title="KPI カード（数値表示）">
        <div className="grid w-full grid-cols-1 gap-3 sm:grid-cols-3">
          {[
            { label: "今月の売上", value: "¥1,240,000", trend: "+12.3%", up: true,  icon: BarChart2 },
            { label: "アクティブユーザー", value: "3,482",       trend: "+5.1%",  up: true,  icon: Users },
            { label: "解約率",     value: "2.4%",        trend: "+0.3%", up: false, icon: TrendingDown },
          ].map((item) => (
            <Card key={item.label}>
      ...
      <DemoSection title="横並び（ユーザー・リスト）">
        <div className="flex w-full max-w-md flex-col gap-2">
          {[
            { name: "山田 太郎",  role: "プロダクトマネージャー", avatar: "YT", active: true },
            { name: "鈴木 花子",  role: "フロントエンドエンジニア", avatar: "SH", active: false },
            { name: "田中 一郎",  role: "デザイナー",            avatar: "TI", active: true },
          ].map((u) => (
            <Card key={u.name}>
              <CardContent className="flex items-center gap-3 p-4">
                ...
                <Badge variant={u.active ? "default" : "secondary"} className="shrink-0 text-xs">
                  {u.active ? "在籍" : "休止"}
                </Badge>
              </CardContent>
            </Card>
      ...
      <DemoSection title="クリッカブルカード（ナビゲーション）">
        <div className="grid w-full grid-cols-1 gap-3 sm:grid-cols-3">
          {[
            { icon: ShoppingCart, title: "受注管理",    desc: "注文一覧・処理状況の確認",  count: "12件" },
            { icon: Users,        title: "顧客管理",    desc: "顧客情報・購買履歴の管理",  count: "3,482件" },
            { icon: BarChart2,    title: "レポート",    desc: "売上・在庫の分析レポート",   count: "5種類" },
          ].map((item) => (
              ...
              <Card className="h-full transition-colors group-hover:border-[var(--color-primary)] group-hover:bg-surface-subtle">
                <CardHeader className="pb-2">
                  ...
                  <CardTitle className="flex items-center justify-between text-sm">
                    {item.title}
                    <ArrowRight className="size-3.5 opacity-0 transition-opacity group-hover:opacity-100" style={{ color: "var(--color-primary)" }} />
                  </CardTitle>
                </CardHeader>
                <CardContent className="flex flex-col gap-1">
                  <p className="text-xs text-muted-foreground">{item.desc}</p>
                  <span className="text-xs font-medium" style={{ color: "var(--color-primary)" }}>{item.count}</span>
                </CardContent>
              </Card>
            </a>
          ))}
      ...
      <DemoSection title="メディアカード（画像付き）">
        <div className="grid w-full grid-cols-1 gap-3 sm:grid-cols-3">
          {[
            { tag: "ヘルスケア", title: "毎日の歩数目標を達成するコツ",   meta: "2024/04/10", read: "3 min" },
            { tag: "レシピ",     title: "簡単10分で作れる朝食アイデア",   meta: "2024/04/08", read: "5 min" },
            { tag: "節約",       title: "食費を月2万円に抑えた方法",       meta: "2024/04/05", read: "4 min" },
          ].map((item) => (
            <Card key={item.title} className="overflow-hidden">
              ...
              <CardHeader className="pb-1 pt-3">
                ...
                <CardTitle className="mt-1 line-clamp-2 text-sm leading-snug">{item.title}</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1"><MapPin className="size-3" />{item.meta}</span>
                  <span className="flex items-center gap-1"><Clock className="size-3" />{item.read}</span>
                </div>
              </CardContent>
            </Card>
          ))}
          ...
          {[
            ...
            >
              <CardContent className="py-3 pl-4 pr-4">
                <p className="mb-0.5 text-xs font-semibold" style={{ color: item.color }}>{item.label}</p>
      ...
      <DemoSection title="コンパクトリスト（アクティビティ）">
        <Card className="w-full max-w-md">
          ...
          <CardContent className="p-0">
            {[
              { user: "山田", action: "タスクを完了しました",   time: "5分前",   avatar: "Y" },
              { user: "鈴木", action: "コメントを追加しました", time: "23分前",  avatar: "S" },
              { user: "田中", action: "ファイルをアップロード", time: "1時間前", avatar: "T" },
              { user: "佐藤", action: "レポートを共有しました", time: "3時間前", avatar: "Sa" },
            ].map((item, i, arr) => (
              <div key={i}>
                <div className="flex items-center gap-3 px-4 py-3">
                  <Avatar className="size-7 shrink-0">
                    <AvatarFallback className="text-xs" style={{ backgroundColor: "var(--color-primary)", color: "#fff" }}>
                      {item.avatar}
                    </AvatarFallback>
                  </Avatar>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-xs text-foreground">
                      <span className="font-medium">{item.user}</span>さんが{item.action}
                    </p>
                  </div>
      ...
      <DemoSection title="レビューカード">
        <div className="grid w-full grid-cols-1 gap-3 sm:grid-cols-2">
          {[
            { name: "佐藤 健",     rating: 5, text: "非常に使いやすく、業務効率が大幅に改善されました。デザインも洗練されていて満足しています。",   date: "2024/04" },
            { name: "中村 美咲",   rating: 4, text: "全体的に良いサービスです。一部の機能に改善の余地がありますが、サポートが丁寧で助かっています。", date: "2024/03" },
          ].map((r) => (
            <Card key={r.name}>
              <CardHeader className="pb-2">
